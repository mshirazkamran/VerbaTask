...
export function Sidebar({ mobileOpen = false, onMobileClose = () => {} }) {
 ...
 <span className="text-sm font-medium text-[#F8FAFC] tracking-tight truncate">
 VerbaTask
 </span>
 ...
 isActive
 ? item.activeClass
 : 'text-[#CBD5E1] hover:text-[#F8FAFC] hover:bg-[#1E293B] border-l-2 border-transparent'
 }`
 }
 ...
 {({ isActive }) => (
 <>
 <Icon className={`w-5 h-5 shrink-0 transition-colors ${isActive ? item.iconActive : 'text-[#94A3B8]'}`} />
 {(!sidebarCollapsed || mobileOpen) && (
 ...
 className="w-full flex items-center gap-3 px-3 py-2 text-xs text-[#94A3B8] hover:text-[#F8FAFC] hover:bg-[#1E293B] rounded-md transition-colors cursor-pointer"
 ...
 <div
 className="fixed inset-0 bg-brand-dark/40 "
 onClick={onMobileClose}
 />
 <div className="relative w-64 max-w-[80vw] h-full shadow-md z-10 animate-slideRight">
 {content}
 </div>
 </div>
 )}
