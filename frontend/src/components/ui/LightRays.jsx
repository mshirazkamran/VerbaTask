...
const LightRays = ({
  ...
  useEffect(() => {
    ...
    const initializeWebGL = async () => {
      ...
      // Cap DPR to 1.25 to prevent fill-rate exhaustion while preserving crisp atmospheric rays
...
float rayStrength(vec2 raySource, vec2 rayRefDirection, vec2 coord,
  ...
  float pulse = pulsating > 0.5 ? (0.85 + 0.15 * sin(iTime * speed * 2.5)) : 1.0;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  if (lightMode > 0.5) {
    vec3 mapped = vec3(1.0) - exp(-max(fragColor.rgb, vec3(0.0)) * 1.5);
    float energy = clamp(max(mapped.r, max(mapped.g, mapped.b)), 0.0, 1.0);
    vec3 hue = mapped / max(energy, 0.0001);
    vec3 ink = mix(hue * 0.15, hue * 0.65, energy);
    fragColor = vec4(ink, clamp(energy * 1.25, 0.0, 0.95));
  }
}
...
void main() {
  ...
  gl_FragColor  = color;
      ...
      const loop = t => {
        ...
        // Pause loop when document is hidden in background tab
